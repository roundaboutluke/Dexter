...
func (c *InfoCommand) Handle(ctx *Context, args []string) (string, error) {
	...
	case "poracle":
		...
		return fmt.Sprintf("Queue info: %s\nQueue summary: %s\nPoracle has been up for %s", queueInfo, queueSummary, formatUptime(uptime)), nil
	case "translate":
		reverse := []string{}
		forward := []string{}
		...
		return fmt.Sprintf("Reverse translation: %s\nForward translation: %s", strings.Join(reverse, ","), strings.Join(forward, ",")), nil
	case "dts":
		if !ctx.IsAdmin {
			return "🙅", nil
		}
...
func infoMoves(ctx *Context) string {
	...
	for _, raw := range ctx.Data.Moves {
		if entry, ok := raw.(map[string]any); ok {
			...
			moves = append(moves, move{Name: name, Type: fmt.Sprintf("%v", entry["type"])})
			nameCounts[name]++
		}
	}
	sort.Slice(moves, func(i, j int) bool { return moves[i].Name < moves[j].Name })
	lines := []string{tr.Translate("Recognised moves:", false)}
	for _, entry := range moves {
		display := entry.Name
		translated := tr.Translate(entry.Name, false)
		if entry.Type != "" && nameCounts[entry.Name] > 1 {
			display = fmt.Sprintf("%s/%s", entry.Name, entry.Type)
			translated = fmt.Sprintf("%s/%s", tr.Translate(entry.Name, false), tr.Translate(entry.Type, false))
		}
...
func infoItems(ctx *Context) string {
	...
	for _, raw := range ctx.Data.Items {
		if entry, ok := raw.(map[string]any); ok {
			items = append(items, fmt.Sprintf("%v", entry["name"]))
		}
	}
	sort.Strings(items)
	lines := []string{tr.Translate("Recognised items:", false)}
	for _, name := range items {
		display := strings.ReplaceAll(name, " ", "\\_")
		translated := strings.ReplaceAll(tr.Translate(name, false), " ", "\\_")
		if translated != display {
			lines = append(lines, fmt.Sprintf("%s or %s", display, translated))
		} else {
			lines = append(lines, display)
...
func weatherStaticMap(ctx *Context, cellID string, weatherID int) string {
	...
	// PoracleJS info weather tiles always use pregenerated tileserver maps (and do not honor staticMapType overrides).
